function onModalWindow(player, modalWindowId, buttonId, choiceId)
	...
	if buttonId == 102 and modalWindowId == 1000 then
		...
		if playerlist then
			...
			player:sendTextMessage(MESSAGE_INFO_DESCR,'Removed '.. string.gsub(" "..ItemType(tonumber(autolootlist[choiceId])):getName(), "%W%l", string.upper):sub(2, 21)..' from auto-loot list.')
		...
		if buttonId == 102 then
			player:sendTextMessage(MESSAGE_INFO_DESCR,'Removed '.. string.gsub(" "..ItemType(itemType:getId()):getName(), "%W%l", string.upper):sub(2, 21)..' from auto-loot list.')
			db.query('DELETE FROM `player_autoloot_persist` WHERE `player_guid` = ' .. getPlayerGUID(player) .. ' AND `item_id` = ' .. itemType:getId() .. '')
			player:removeAutoLootItem(itemType:getId())
			...
			if playerlist and #playerlist >= limiteAutoloot then
				...
				player:sendTextMessage(MESSAGE_INFO_DESCR,'Added '.. string.gsub(" "..ItemType(lootBlockList[choiceId].itemId):getName(), "%W%l", string.upper):sub(2, 21) ..' to auto-loot list.')
...
function call(player, param, param2, tobpid)
	...
	if param == 'add' then
		...
		if getContainerSize(getPlayerSlotItem(player, CONST_SLOT_BACKPACK).uid) then
			...
			player:sendCancelMessage("You do not have a main backpack.")
			return false
		end
		if sum == 1 then
			player:sendCancelMessage("You do not have sub-backpacks.")
			return false
		end
